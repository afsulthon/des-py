...
# Enkripsi
def encrypt(text, round_key, round_key_hex):
	# Konversi string heksadesimal ke biner
	text = hex2bin(text)

	# Permutasi awal
	text = permute(text, IP)
	print("Setelah permutasi awal:", bin2hex(text), "\n")
	...
	for i in range(16):
		...
		print("Ronde", i + 1, "→", bin2hex(left), bin2hex(right), round_key_hex[i])
...
# Teks yang akan dienkripsi (dalam 64-bit heksadesimal)
text = "A1B4C7D2E5F80913"
# text = input("Masukkan teks yang akan dienkripsi: ")
# if len(text) != 16:
# 	print("Eror: Teks harus berupa 64-bit heksadesimal")
# 	exit()

# Kunci eksternal enkripsi (dalam 64-bit heksadesimal)
key = "2F6A8D1B3C7E5904"
# key = input("Masukkan kunci eksternal enkripsi: ")
# if len(text) != 16:
# 	print("Eror: Teks harus berupa 64-bit heksadesimal")
# 	exit()
...
round_key = []		# dalam biner
round_key_hex = []	# dalam heksadesimal
...
for i in range(16):
	...
	round_key_hex.append(bin2hex(cur_rk))

print("\n【 ENKRIPSI 】\n")
cipher_text = bin2hex(encrypt(text, round_key, round_key_hex))
print("=> Cipher text:", cipher_text)

print("\n【 DEKRIPSI 】\n")
round_key_rev = round_key[::-1]
round_key_hex_rev = round_key_hex[::-1]
text = bin2hex(encrypt(cipher_text, round_key_rev, round_key_hex_rev))
